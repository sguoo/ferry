...
| **환경설정** | 저장 폴더, 파일명 템플릿, 기본 화질·컨테이너·자막 언어, 동시 다운로드, 속도 제한, 클립보드 감지, 다른 화면에서도 계속 재생, 완료 알림/사운드, ffmpeg 경로(즉시 검증). 저장 시 검증 실패 항목은 인라인 오류로 표시. |
| **YouTube 계정 연동** | "봇이 아님을 확인"·연령 제한·회원 전용 영상은 로그인 세션이 필요합니다. 환경설정 › YouTube 계정 연동에서 **cookies.txt 파일**(권장: 로그인한 브라우저에서 'Get cookies.txt LOCALLY' 확장으로 내보내기) 또는 브라우저 직접 읽기(Firefox 안정, 최신 Chrome/Edge는 실행 중·암호화 때문에 자주 실패)를 지정하고 `연결 테스트`로 쿠키 수와 로그인 세션을 확인합니다. 해당 오류가 나면 배너의 `쿠키 연결하러 가기`가 이 섹션으로 바로 이동하고, 저장하면 실패했던 요청을 자동으로 다시 시도합니다. CLI는 `--cookies FILE` / `--browser chrome`. |
